...
def metpotI2(A,mu,tol=1e-8,maxrep=np.inf):
   ...
   iX = tf.calcula_inversa(X) # La invertimos
...
def laplaciano_iterativo(A,niveles,nombres_s=None):
    ...
    if A.shape[0] == 1 or niveles == 0: # Si llegamos al último paso, retornamos los nombres en una lista
        ...
        v,l,_ = metpot2(L,v1,l1,tol=1e-8,maxrep=np.inf) # Encontramos el segundo autovector de L
        ...
        for i in range(len(v)):
            if v[i] < 0:
                ...
                neg.append(A[i,:])
                pos.append(0*A[i,:])
        Ap = np.array(pos) # Asociado al signo positivo
        Am = np.array(neg) # Asociado al signo negativo
        ...
        if Q0<=0 or all(v>0) or all(v<0): # Si la modularidad actual es menor a cero, o no se propone una partición, terminamos
            ...
            for i in range(len(v)):
              if v[i] < 0:
                ...
                neg.append(R[i,:])
                pos.append(0*A[i,:])
            Rp = np.array(pos)  # Parte de R asociada a los valores positivos de v
            Rm = np.array(neg) # Parte asociada a los valores negativos de v
            ...
            if Q0 >= Q1: # Si al partir obtuvimos un Q menor, devolvemos la última partición que hicimos
                ...
                print("Nombres positivos:", nombres_p)
                print("Nombres negativos:", nombres_m) 
